...
Here the characteristics of the haul should be collcted as follow:\

- **Station**: station number
- **DataTime**: date in the following format: "DAY/MONTH/YEAR"
- **HourTimeStart**: start time of the haul in HOURS:MINUTES
- **Lat(°)Start**: Latitute degrees (e.g. 45) of the haul deployment
- **Lat(')Start**: Latitute primes and decimals (e.g. 36.123) of the haul deployment
- **Lon(°)Start**: Longitude degrees (e.g. 12) of the haul deployment
- **Lon(')Start**: Latitute primes and decimals (e.g. 48.151) of the haul deployment
- **Wal(m)**: Length of the cables
- **Depth Start (m)**: Depth at the start of the haul
- **Speed (kn)**: trawl speed
- **Route**: Route in degrees
- **WindDirection**: follow the code on the sheet "Note di compilazione"
- **SeaCondition**: follow the code on the sheet "Note di compilazione"
- **WeatherCond**: follow the code on the sheet "Note di compilazione"
- **BottomType**: Main benthonic species composition, according to Medits codes  (e.g. ANADTRA)
- **Notes**: any further note (e.g. twisted cables)
- **HourTimeStart**: start time of the haul in HOURS:MINUTES
- **Lat(°)End**: Latitute degrees (e.g. 45) of the end of the haul 
- **Lat(')End**: Latitute primes and decimals (e.g. 36.123) of the end of the haul
- **Lon(°)End**: Longitude degrees (e.g. 12) of the end of the haul
- **Lon(')End**: Latitute primes and decimals (e.g. 48.151) of the end of the haul
- **Depth End (m)**: Depth at the end of the haul


They have also been uploaded to the ‘Notes for compilation’ page.
...
- **WRapA(kg)**: the weight of rapid A
- **Tara A (kg)**: the respective tare weight, if present;
- **WRapD(kg)**: the weight of rapid A
- **Tara D (kg)**: the respective tare weight, if present;
- **CodBenthos**:the code inserted inside the benthos bag, which will be used to trace it back to the relevant fishing ground;
- **WBenthos(kg)**:the weight of the benthos bag, which must be at least 6 kg;
...
- **Litter_saccata(kg)**: the weight of the litter in the bag;
- **Litter_gabbia(kg)**: the weight of very heavy litter (e.g.: a large log or a large steel pole). This should be indicated in a separate section as it would affect the RF calculation.
- **Notes**: any notes;
- **Notes shells**: indicate here whether certain commercial species must be derived from the benthos bag (e.g. BOLINUS);

**IMPORTANT: If the catch contains large numbers of commercial shells such as Aequipecten opercularis, Flexopecten glaber, Bolinus brandaris, Hexaplex trunculus, Rapana venosa, it is advisible to estimate them from benthos. In case of big amount of shells, when it is feasible to collect all the individuals of one or more "shell" commercial species, please use the samples onboard sheet to collect the data and DO NOT bring it in lab. Conversely, if they are few, they can be collected with the other commercial species.**

![Figure 2: NotesCala](C:/Users/f.luzi/OneDrive - CNR/CNR/SoleMon/Github/SoleMon_project/OnBoard/data/other/NotesCala.png)

## Samples onboard

This sheet should be used to record the single species data collected onboard as follow:

- **Gear**: Geard ID (A or D)
- **Species**: Use the drop-down menu to select the species
- **Length_mm**: to be filled only in case of individual length data collection
- **Weight_g**: to be filled only in case of individual weight data collection. If it is not possible to collect individual weight for target species leave it blank and it will be estimate in post processing. 
Therefore, DO NOT COLLECT total weight in case of commercial species with single measurements.
- **Sex**: to be filled only in case of individual data collection (M, I or F)
- **Mat**: to be filled only in case of individual data collection using the correct scale based on species
- **Number**: to be filled only in case of species without single individual measurements (e.g. BOLINUS). If a proportion is required, calculate it on board and report the final result
- **Total weight_g**: to be filled in case of counted number of individuals (e.g. BOLINUS) or in case of single length measurements collected for secondary species (e.g. ARNOLAT). In the second case please insert total weight in correspondence with the last length measurement.

![Figure 3: samples_onboard](C:/Users/f.luzi/OneDrive - CNR/CNR/SoleMon/Github/SoleMon_project/OnBoard/data/other/samples_onboard.png)

## Species

This sheet contains the list of species that can be eventually collected onboard used for the drop-down menu in "Samples onboard" sheet.

![Figure 4: Species](C:/Users/f.luzi/OneDrive - CNR/CNR/SoleMon/Github/SoleMon_project/OnBoard/data/other/Species.png)
...
###  **Discard subsample**
It is a way to help the benthos data collection in lab and avoid possible misspecification that can happen in doing proportion by hand. The proportion are automatically computed in post processing if the access benthos is filled correctly.


#### **Benthos 1 - typical situation**
All individuals of the species (e.s:ANADTRA) are collected from the benthos subsample (the benthos bag) and weighed. Then a sub-sample (50/100 individuals) of species is weighed and counted (Es: Ophiura). The sample weight of all individuals of the species is entered in *weight_g*, the number of individuals of species taken for the subsample is entered in *subsample_numbers* and the subsample weight of species is entered in *kg_field3*;
...
#### **Benthos 2 - TURRITELLA**

All individuals of the species are collected from the benthos subsample (the benthos bag) and weighed.  Then a sub-sample of 50/100 individuals is taken and weighed. The shells of these individuals are broken and divided into three categories: empty shells or shells containing sand (shells) and shells containing a marine organism. This case to be with the species: *Turritella communis*(B) and *Aporrhais pespelecani*(B), in their shells can be *Sipuncula*(A). The sample weight of all collected samples is entered in *kg_field1*, the subsample weight (weight of 50/100 individuals) is entered in *kg_field2*. Once all shells have been broken up and each subsample (e.g. shell, turritella and sipuncula) has been counted and weighed, the subsample weight divided by groups is entered in *kg_field1* and the number in *subsample_numbers*. Different records must be created for the different species we find by entering each information in each record (Subsample_numbers, Kg_field1, Kg_field2, Kg_field3);

![Figure 16: Discard subsample, case Turritella](C:/Users/f.luzi/OneDrive - CNR/CNR/SoleMon/Github/SoleMon_project/OnBoard/data/other/turritella_bb.png)
